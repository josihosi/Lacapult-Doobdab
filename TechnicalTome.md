...
The companion proof tools are `tools/caol_mod_status_model.py`, `tools/prove_caol_mod_status_model.py`, and `tools/godot_caol_mod_status_smoke.gd`. The Python proof creates a sandbox fixture under ignored `.proof-cache/`, covers stock/user/catalog/world-custom sources plus enabled/disabled, obsolete, broken metadata, dependency-blocked, summary-ready/missing/partial/not-needed/unknown, and generated-pack-present states, and emits `.proof-cache/caol-mod-status/status.json`. The Godot smoke runs the same fixture through the autoloaded `CaolModStatus` implementation and writes `.proof-cache/caol-mod-status/godot-status.json`. Slice 1 does not install or enable mods, apply generated packs, mutate real user data, call any backend, or prove C-AOL runtime consumption.

## 2026-04-25 C-AOL mod/Summarizer UX status Slice 2

`CaolModStatus.build_ux_overview(status)` converts the Slice 1 status dictionary into a read-only launcher view model: installed/custom counts, enabled/disabled counts, obsolete/metadata/dependency blocker counts, summary-ready/missing/partial/unknown/not-needed counts, an `all_enabled_extra_context_state` of `all-clear`, `needs-summaries`, `blocked`, or `unknown`, and short candidate briefs for dry-run Summarizer prompts. `CaolModStatus.build_dry_run_summarizer_prompt(status, backend_mode, backend_status)` returns a status-only action object whose side-effect flags stay false (`would_mutate`, `would_call_backend`, `would_generate_pack`, `would_enable_mods`).

`ModManager.get_caol_mod_summarizer_overview()` and `get_caol_summarizer_dry_run()` expose that view model to `scripts/ModsUI.gd` and `scripts/SettingsUI.gd`. The Mods tab now adds summary badges to installed rows, appends C-AOL Summarizer status to selected-mod details, and posts a post-install dry-run prompt when enabled contextual mods lack complete summaries. A true post-enable hook remains pending because Lacapult does not yet own C-AOL world `mods.json` enable/apply changes. The Settings bridge now renders live overview text and a `Summarizer dry-run status` button instead of a fixed packaged-mod report string. This is still Slice 2 only: the UI prompt does not call a backend, generate packs, apply generated summaries, enable mods, edit `mods.json`, or mutate real user data.

## 2026-04-25 C-AOL summary-pack apply/rollback Slice 3
...
## 2026-04-27 Ollama setup workflow tightening

`scripts/BackendSetupUI.gd` now keeps Ollama mode to one visible model selector (`Ollama model choice`) with the supported Mistral/Nemotron choices; the old freeform `Ollama model tag` row is hidden in Ollama mode so there are not two competing model controls. Ollama status is shown through compact lights for command, server, Mistral, Nemotron, Python/venv, and options-save readiness.

`scripts/BackendConfigManager.gd` owns fixtureable Ollama readiness (`LACAPULT_OLLAMA_FIXTURE`) for command-missing, server-unreachable, model-present, and model-missing proof states. `Check` remains detect-only. `Install Ollama / model` saves current options first, then requires confirmation before a platform-specific setup/model-pull plan can run; automated proof mode records `caol_ollama_setup_intent.json` only. `Install venv` is separately confirmation-gated because C-AOL launches `tools/llm_runner/runner.py` through Python for Ollama too; automated proof records `caol_python_venv_setup_intent.json` only and does not create a venv.

The repeatable proof is `tools/godot_ollama_workflow_smoke.gd`, run with isolated `HOME`. It proves the one-control UI, Mistral/Nemotron readiness lights, fixture model-present/missing/error states, Check no-write behavior, Save config/options round-trip, and confirmation-gated Ollama/model plus Python-venv setup intents without installing Ollama, creating a venv, pulling models, calling APIs, or mutating real user data.

## 2026-04-27 window chrome investigation footing

Lacapult is not currently using native OS window chrome: `project.godot` sets `display/window/size/borderless=true`, and `scenes/Catapult.tscn` reserves a top offset for a custom `TitleBar` instance from `scenes/CustomTitleBar.tscn`. That scene owns TextureButton Minimize/Maximize/Close controls, so close/minimize/top-bar appearance complaints should be treated first as custom scene UI / DPI-scaling issues, not native macOS/Windows chrome issues.

The first Package 5 fix tightened the smallest custom metric seam: titlebar `32px -> 28px`, `Main.margin_top 36px -> 32px`, app icon `24x24 -> 20x20`, close/min/max buttons `32x24 -> 28x20`, and vertical titlebar margins `4px -> 2px`. Repeatable local evidence: `HOME=$(mktemp -d /tmp/lacapult-window-chrome-home.XXXXXX) godot --path . --no-window -s tools/godot_window_chrome_inspection.gd`. This proves the local scene seam changed; Windows visual confirmation remains separate before a cross-platform fix claim.
