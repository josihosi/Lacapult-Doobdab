...
func _run() -> void:
	...
	_require(ui._confirm_dialog.dialog_text.find("Step 1:") >= 0 and ui._confirm_dialog.dialog_text.find("download uv") >= 0, "API setup confirmation missing uv download phase")
	_require(ui._confirm_dialog.dialog_text.find("install_managed_cpython") >= 0 or ui._confirm_dialog.dialog_text.find("CPython") >= 0, "API setup confirmation missing managed CPython phase")
	_require(ui._confirm_dialog.dialog_text.find("tools/openclaw_harness/requirements.txt") >= 0, "API setup confirmation missing harness requirements phase")
	_require(ui._confirm_dialog.dialog_text.find("pip install --python") >= 0 and ui._confirm_dialog.dialog_text.find("any-llm-sdk[openrouter]") >= 0, "API setup confirmation missing AnyLLM package phase")
