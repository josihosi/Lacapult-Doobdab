...
7. Park next slices
   ...
   - [active] Real C-AOL mod/Summarizer generation/apply UI v0: Settings apply-preview, confirmed writer/apply seam, first backend-generation seam, and Settings target world/mod chooser error polish are landed and sandbox-smoked; next bounded step is optional real-local Ollama generation smoke behind the same confirmation/readiness gates, or final retestable-state polish if Schani does not want live local-model evidence yet. Still no secrets, model pulls, package installs, or real Application Support mutation in automated proof.
   - [queued] Lacapult post-mod UI Windows retest release packet: after the current mod/Summarizer UI lane is honestly retestable, regenerate fresh unsigned packages and publish a Lacapult GitHub test/prerelease so Josef can download the Windows build from Releases. Canonical contract: `doc/lacapult-post-mod-ui-windows-retest-release-packet-2026-04-26.md`.
