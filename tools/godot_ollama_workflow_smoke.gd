...
func _run() -> void:
	...
	_require(missing.get("model_states", {}).get("mirage335/NVIDIA-Nemotron-Nano-9B-v2-virtuoso:latest", "") == "yellow", "fixture model-missing state did not mark Nemotron pull-needed")
	...
	print("  UI proof: one short-label Ollama model-choice control, real Mistral/Nemotron tags, GiB hardware/performance lights, Check, Save, Install Ollama / model, and Create venv only rendered")
