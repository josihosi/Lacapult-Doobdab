...
Catapult-Dabubu Windows-first install/release drift audit evidence landed on 2026-05-20:

- Live C-AOL release proof: `python3 tools/prove_caol_release.py --all-platforms` verified the current GitHub releases list after new Linux-only `caol-cdda-master-2026-05-18-*` prerelease rows appeared. The launcher now prefers the exact cross-platform `v0.2.0` row for Linux, macOS, and Windows, then keeps installable curated rows ahead of blocked/no-platform-asset rows.
- Windows install/launch proof: `python3 tools/prove_windows_install_launch_static.py` verifies the Windows release filter selects `_windows.zip`, the installer accepts `Cataclysm-AOL.exe`, and the Windows Play path discovers the installed C-AOL executable instead of hardcoding `cataclysm-tiles.exe`.
- Live Windows archive proof: `python3 tools/prove_caol_windows_archive_shape.py` verified the selected `v0.2.0` Windows archive downloaded into ignored `.proof-cache/caol-live-windows/`: 300,287,206 bytes, SHA-256 `61fe4da4a644573d4970a22962ddf2249db121aa4dd46b65b7e399f7e19f5917`, 7,823 ZIP entries, CRC clean, root `cataclysm-tiles.exe`, C-AOL runner `tools/llm_runner/runner.py`, and required `data/json/`, `gfx/`, and `lang/mo/` content.
- Regression proof passed: `python3 tools/prove_windows_retest_followup_v2_backend_static.py`; `python3 tools/prove_caol_backend_contract.py`; `git diff --check`; `PYTHONPYCACHEPREFIX=/private/tmp/catapult-pycache python3 -m py_compile tools/prove_caol_release.py tools/prove_windows_install_launch_static.py tools/prove_caol_windows_archive_shape.py`.
- Godot/macOS boundary: project load and export packaging passed outside the Codex macOS sandbox; the in-sandbox Godot `pixelFormat == nullptr` crash is a sandbox graphics-context issue, not evidence that the app cannot run on macOS.
- Package proof: `python3 tools/prove_lacapult_export_packaging.py` exported unsigned local macOS/Linux/Windows packages; Windows package `.proof-cache/lacapult-export/packages/Catapult-Dabubu-windows-unsigned.zip` was 66,658,105 bytes with SHA-256 `f86ff6a31efeed0e393706512b541b6029562af9f3c6153f4cc6297d05fd65eb`.
- Remaining boundary: this audit did not extract into a real launcher install directory and did not perform a real Windows first-launch click-through.

Catapult-Dabubu C-AOL mod enable/Summarizer flow evidence landed on 2026-05-20:

- LLM options handoff fix: `Save options` now writes `caol_backend_setup.json`, `caol_llm_options_patch.json`, `caol_llm_options_apply.json`, and applies the selected runner settings into the active C-AOL userdir `config/options.json` with a backup under `caol/save_backups/`. The applied C-AOL options include `LLM_INTENT_ENABLE=true`, backend mode, API-vs-local mode, API provider/model/env-var or Ollama URL/model, and the Python runner path.
- C-AOL provider fix: `/Users/josefhorvath/Schanigarten/Cataclysm-AOL/src/llm_intent.cpp` now reads `LLM_INTENT_API_PROVIDER` with an OpenAI fallback instead of hardcoding `openai`, so launcher provider selection can reach C-AOL runner.py/AnyLLM.
- Active-options proof passed: `tools/godot_api_anyllm_workflow_smoke.gd`; `tools/godot_ollama_workflow_smoke.gd`; `tools/godot_v3_save_apply_runner_smoke.gd`; `tools/godot_backend_setup_save_check_smoke.gd`; `tools/godot_backend_triad_smoke.gd`; and `tools/godot_windows_retest_followup_v1_smoke.gd` under isolated `HOME` values outside the macOS sandbox. These smokes inspect active isolated `config/options.json`, not only launcher-side metadata.
- Static/backend proof passed: `python3 tools/prove_caol_backend_contract.py`; `python3 tools/prove_windows_retest_followup_v2_backend_static.py`; `python3 tools/prove_caol_summarizer_ui_polish.py`; py_compile for touched Python proof scripts; Godot project load outside the macOS sandbox; and `git diff --check` in both Catapult-Dabubu and the touched C-AOL source file.
- Current C-AOL port mod inventory proof: `python3 tools/prove_caol_mod_inventory.py --caol-root /Users/josefhorvath/Schanigarten/Cataclysm-AOL --report-dir .proof-cache/caol-mod-bridge-current` inspected the current `port/cdda-master` tree. It found 40 non-obsolete packaged stock mods and 6 obsolete-blocked mods; 31 are mechanically path-supported but need generated C-AOL summary packs, 9 need no summary, and none were classified as already summary-ready in the stock tree.
- Remaining boundary: local/current-tree inventory is static compatibility evidence. It does not run every mod in-game, install arbitrary external mod repos, or live-call API/Ollama generation with real secrets.

- Root-cause check: prior C-AOL mod work proved status and sandbox summary apply, but the normal launcher flow could still strand stock JSON mods such as Magiclysm/DinoMod as visible-but-disabled. Selecting a stock C-AOL JSON mod through the Mods tab could be skipped as stock/no-op, so no world `mods.json` change happened and the Summarizer never saw an enabled source mod.
- Launcher fix: C-AOL now has a confirmed world-enable path that writes the selected world's `mods.json` with dependency ordering and backup visibility before any Summarizer generation. Settings now exposes `Enable selected mod in world`; the Mods tab routes C-AOL JSON catalog selections through the same enable path instead of treating stock packaged mods as un-actionable.
- Summarizer backend fix: the confirmed generation bridge no longer treats API mode as the same gated stub as OpenVINO. When API readiness passes, it routes summary generation through the existing `any-llm-sdk[...]` / `from any_llm import completion` seam, uses only the configured env-var name for the session API key, normalizes empty generated fields, and still writes nothing if import/model/key checks fail.
- New Godot flow proof: `HOME=$(mktemp -d /tmp/lacapult-caol-mod-enable-home.XXXXXX) godot --audio-driver Dummy --video-driver GLES2 --disable-render-loop --path . --no-window -s tools/godot_caol_mod_enable_summarizer_flow_smoke.gd` proves disabled stock Magiclysm becomes enabled in the sandbox world, then becomes a summary candidate, then receives a fixture-generated C-AOL-native companion pack; it also proves DinoMod enable writes dependency order `dda`, then `DinoMod`.
- Regression proof passed: `python3 tools/prove_caol_summarizer_ui_polish.py`; `python3 tools/prove_caol_mod_status_model.py`; `python3 tools/prove_caol_summary_pack_apply.py`; `python3 tools/prove_caol_summary_error_matrix.py`; `python3 tools/prove_windows_retest_followup_v2_backend_static.py`; `tools/godot_v3_mod_procedure_smoke.gd`; `tools/godot_api_anyllm_workflow_smoke.gd`; `tools/godot_ollama_workflow_smoke.gd`; Godot project load outside the macOS sandbox; `git diff --check`; and py_compile for touched Python proof scripts.
- Remaining boundary: automated Godot flow proof still uses isolated HOME plus fixture backend for generation, and the API path is static-proven rather than live-secret-tested. Real Windows click-through should verify the selected world, enable action, Summarizer preview, and backend-ready API/Ollama generation path against a real C-AOL install.
...
- Source/static proof: `python3 tools/prove_caol_backend_contract.py` verifies C-AOL exposes `LLM_INTENT_ENABLE`, backend/model/Python/env-var options, hidden `LLM_INTENT_USE_API`/provider metadata, and that Catapult's patch builder includes runner enablement plus selected backend/model/mode without secret-bearing fields. The C-AOL runtime now reads `LLM_INTENT_API_PROVIDER` instead of hardcoding OpenAI.
- Godot UI/sandbox proof: `HOME=$(mktemp -d /tmp/lacapult-v3-save-apply-home.XXXXXX) godot --path . --no-window -s tools/godot_v3_save_apply_runner_smoke.gd` exercises API and Ollama `Save options`, proves active isolated C-AOL userdir `config/options.json` is updated, and then applies the same option set to sandbox artifacts. API proof sets `LLM_INTENT_ENABLE=true`, `LLM_INTENT_BACKEND=api`, `LLM_INTENT_USE_API=true`, selected API model/provider/env-var, and Python runner path. Ollama proof normalizes Nemotron to `nemotron-9b-dumber:latest` and sets `LLM_INTENT_ENABLE=true`, `LLM_INTENT_BACKEND=ollama`, `LLM_INTENT_USE_API=false`, selected URL/model, and Python runner path.
...
- Safety boundary: no live API call, API secret readout, package-manager install, Python venv creation, Ollama request/model pull, public release action, or non-isolated Application Support/user-data mutation was performed.
...
- Godot Ollama UI smoke: `HOME=$(mktemp -d /tmp/lacapult-v3-ollama-home.XXXXXX) godot --path . --no-window -s tools/godot_ollama_workflow_smoke.gd` proves the short-label Ollama UI still renders, direct Nemotron plan normalizes `nemotron-9b` to `nemotron-9b-dumber:latest`, Save options also rewrites an old/source Nemotron setting to that no-think runtime alias in active isolated `options.json`, preserves source `mirage335/NVIDIA-Nemotron-Nano-9B-v2-virtuoso:latest`, writes a `/no_think` Modelfile, creates the no-think alias, and records proof-mode setup intent without installer, venv creation, model pull, alias create, API call, or non-isolated user config mutation.
