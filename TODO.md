...
7. Park next slices
   ...
   - [active] Real C-AOL mod/Summarizer generation/apply UI v0: first Settings apply-preview plan is landed and sandbox-smoked; next bounded step is the actual confirmed sandbox writer/generation seam that stages a C-AOL-native companion pack from the preview, preserves backup/rollback visibility, and still never mutates real Application Support state in automated proof.
