...
Lacapult is not currently using native OS window chrome: `project.godot` sets `display/window/size/borderless=true`, and `scenes/Catapult.tscn` reserves `Main.margin_top = 36` for a custom `TitleBar` instance from `scenes/CustomTitleBar.tscn`. That scene owns TextureButton Minimize/Maximize/Close controls sized `32x24` with SVG expansion/stretch, so close/minimize/top-bar appearance complaints should be treated first as custom scene UI / DPI-scaling issues, not native macOS/Windows chrome issues.

Repeatable local evidence: `HOME=$(mktemp -d /tmp/lacapult-window-chrome-home.XXXXXX) godot --path . --no-window -s tools/godot_window_chrome_inspection.gd`. This proves the root-cause class locally only; Windows visual confirmation remains separate before a cross-platform fix claim.
