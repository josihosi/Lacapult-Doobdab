...
func _on_ConfirmPythonVenvInstall_pressed() -> void:
	...
	if result != "ok":
		_set_backend_status(Settings.read("backend_mode"), result, "Python venv save failed")
		return
...
func _on_ExternalBackendAction_confirmed() -> void:
	...
	if _pending_confirm_action == "api":
		...
		if not proof_only:
			_set_external_action_progress("api", "Creating/updating venv, then installing harness requirements and AnyLLM/provider packages. Keep Catapult-Dabubu open; this may take a while.")
		...
		if setup_result.get("proof_only", false):
			...
			post_message = "C-AOL API backend setup command finished; venv, harness requirements, and AnyLLM/provider packages were handled after confirmation. No API call or API-secret read was performed."
...
func _set_external_action_progress(mode: String, message: String) -> void:
	...
	if mode == "api" and _api_status_lights != null:
		_set_status_rows(_api_status_lights, [
			...
			{"label": "Harness requirements", "state": "yellow"},
