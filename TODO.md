...
7. Park next slices
   ...
   - [x] Real C-AOL mod/Summarizer generation/apply UI v0: Settings apply-preview, confirmed writer/apply seam, first backend-generation seam, Settings target world/mod chooser error polish, and optional live-local Ollama smoke are landed and sandbox-smoked. The live smoke used already-local `mistral:latest` through Ollama HTTP and applied the generated entry through the same isolated companion-pack seam. Still no secrets, model pulls, package installs, remote APIs, or real Application Support mutation in automated proof.
