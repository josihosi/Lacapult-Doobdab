...
func _run() -> void:
	...
	yield(self, "idle_frame")
	_require(ui._confirm_dialog.dialog_text.find("uv 0.11.19") >= 0 and ui._confirm_dialog.dialog_text.find("tools/openclaw_harness/requirements.txt") >= 0 and ui._confirm_dialog.dialog_text.find("pip install --python") >= 0 and ui._confirm_dialog.dialog_text.find("any-llm-sdk[openrouter]") >= 0, "confirmation did not show planned uv + harness + any-llm-sdk setup phases")
	_require(ui._confirm_dialog.dialog_text.find("app-managed CPython") >= 0 and ui._confirm_dialog.dialog_text.find("harness requirements") >= 0 and ui._confirm_dialog.dialog_text.find("installs AnyLLM") >= 0, "confirmation did not explain real uv/venv/harness/package install boundary")
