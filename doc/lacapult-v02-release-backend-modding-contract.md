...
Modding is now split into the next active implementation family in `doc/lacapult-mod-summarizer-feature-plan-2026-04-25.md`. Current Lacapult modding status remains read-only packaged-mod inventory plus C-AOL-native summary-root guidance, but the next lane is feature-complete UX, not a hidden metadata trick: after mod install/enable, Lacapult should offer a Summarizer button/pop-up, show whether extra NPC/content summaries exist, track generation/apply status, write/apply C-AOL-native `Summaries_short` / `Summaries_extra` data, and prove C-AOL runtime summary loading sees it in a sandbox. Schema/runtime behavior stays on the C-AOL side; Lacapult is the UX/status/apply helper. Do not regress the backend-good hardening while implementing this, because the soup is already trying to become architecture.
