...
- The player-facing copy distinguishes the important layers: Lacapult install/copy succeeded, the C-AOL app bundle exists, but the actual launch binary is blocked by non-portable local dylib dependencies. It explicitly says this is a C-AOL macOS package portability issue that Lacapult can report, not repair.
...
- This remains launcher-side UX/status only. It does not patch/rebuild the C-AOL package, install missing dylibs, sign/notarize, publish releases, contact upstream, use API secrets, pull models, or mutate Josef's real Application Support install state.
...
## Evidence - 2026-04-25 C-AOL mod/Summarizer Slice 2 UX dry-run status

- Added a Slice 2 UX view model on top of `scripts/CaolModStatusModel.gd` and surfaced it through `ModManager.get_caol_mod_summarizer_overview()` / `get_caol_summarizer_dry_run()`.
- Mods tab now renders a read-only C-AOL Summarizer status strip, compact installed-mod badges for enabled/disabled plus summary/blocker state, selected-mod detail text, and a `Summarizer dry-run` action. Settings now renders the same live read-only status rather than only the static packaged-mod report and has a dry-run/status-only button.
- The dry-run action reports enabled contextual mods that would later need C-AOL-native companion summary packs plus the selected backend readiness gate. It explicitly reports no backend call, no generated pack, no pack apply, no mod enable, and no save/userdata mutation. The install path posts the same status prompt after mod install when candidates exist; no inherited mod-enable flow exists yet, so enable-hook prompting remains a documented gap.
- Re-ran `python3 tools/prove_caol_mod_status_model.py`; it rebuilt the ignored sandbox fixture/report under `.proof-cache/caol-mod-status-fixture` / `.proof-cache/caol-mod-status/status.json` and preserved the Slice 1 status assertions.
- Added and ran `tools/godot_caol_mod_ux_status_smoke.gd` with `LACAPULT_CAOL_MOD_STATUS_FIXTURE="$PWD/.proof-cache/caol-mod-status-fixture"` and `LACAPULT_CAOL_MOD_UX_OUTPUT="$PWD/.proof-cache/caol-mod-status/ux_status.json"`. It proved the rendered overview state is `needs-summaries`, at least one dry-run candidate exists, and the dry-run action is `summarizer_dry_run_status_only` with all side-effect flags false.
- Re-ran `tools/godot_caol_mod_status_smoke.gd` against the same fixture; it still reported 10 fixture mods, 4 enabled, 2 summary-ready, and 4 summary-missing. Expected fixture JSON parse errors for the intentionally broken metadata mod were printed by Godot and did not fail the proof.
- Re-ran `/opt/homebrew/bin/godot --path . --no-window --quit`; Godot 3.6.2 exited 0 with the known macOS/headless icon/resource cleanup warnings.
- No API secrets, backend calls, model pulls/downloads, OpenVINO installs, generated summary packs, real Application Support saves/mods/config, signing, notarization, tags, releases, or public release publication were used.
