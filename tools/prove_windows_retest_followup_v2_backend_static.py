...
require('const OLLAMA_MODEL_NEMOTRON_SOURCE = "mirage335/NVIDIA-Nemotron-Nano-9B-v2-virtuoso:latest"' in backend + ui, "Nemotron source pull tag is missing")
require('const OLLAMA_MODEL_NEMOTRON = "nemotron-9b-dumber:latest"' in backend + ui, "Nemotron runtime tag is not the no-think local alias")
require('SYSTEM \\"/no_think' in backend and 'create_nemotron_no_think_alias' in backend and '["create", OLLAMA_MODEL_NEMOTRON, "-f", modelfile_path]' in backend, "Nemotron setup does not create a SYSTEM /no_think alias")
require('reuses the source model blobs instead of pulling a second copy' in backend or 'does not duplicate model weights' in backend, "Nemotron alias setup does not document no-duplicate-weight boundary")
...
require('_windows_gpu_names' in backend and '_ollama_acceleration_state' in backend and '_cap_ollama_performance_for_acceleration' in backend, "Ollama hardware check does not classify acceleration/fallback state")
require('CPU-only slow fallback' in backend and 'iGPU/other GPU slow fallback' in backend and 'NVIDIA/CUDA accelerated' in backend, "Ollama fallback labels for CPU/iGPU/NVIDIA are missing")
require('RAM: %.1f GiB   VRAM: %.1f GiB   Acceleration: %s' in ui, "Ollama hardware UI does not render GiB plus acceleration values")
require('"label": "Acceleration"' in ui, "Ollama acceleration status row missing")
...
require('C_AOL_OPTIONS_APPLY_FILENAME = "caol_llm_options_apply.json"' in backend, "active C-AOL options apply manifest file is missing")
require('_apply_caol_options_patch_to_active_user_config' in backend and 'config/options.json' in ui, "Save options no longer advertises/applies active C-AOL options.json")
...
print("  Ollama selector labels are short; Nemotron prepares a no-think alias from the Virtuoso source")
print("  hardware display uses GiB, acceleration fallback state, and model-specific performance lights")
print("  package setup failures surface non-secret command output summaries")
print("  Save options applies runner settings to active C-AOL options.json with an apply manifest")
