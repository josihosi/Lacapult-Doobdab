...
func _run() -> void:
	...
	settings.store("backend_ollama_model", "nemotron-9b")
	...
	_require(all_text.find("RAM 32000 MB / VRAM 12000 MB") >= 0, "hardware fixture check did not render measured RAM/VRAM")
	...
	_require(_find_status_row(ui._ollama_status_lights, "HardwarecheckStatusRow", "green"), "hardware check did not expose green explicit state")
	...
	var missing_plan = backend_config.build_ollama_setup_plan("http://127.0.0.1:11434", "nemotron-9b")
	...
	print("  Status UI: explicit colored big-dot rows for API/Ollama/hardware, no emoji traffic lights")
	print("  Ollama: fixture hardware check, timeout/wait warning, serialized setup plan, no install+pull chain when not ready")
	quit(0)
